...
void maxLimit(double max) {
  if (left1Speed > max)
    left1Speed = max;
  else if (left1Speed < -max)
    left1Speed = -max;
  if (left2Speed > max)
    left2Speed = max;
  else if (left2Speed < -max)
    left2Speed = -max;
  if (right1Speed > max)
    right1Speed = max;
  else if (right1Speed < -max)
    right1Speed = -max;
  if (right2Speed > max)
    right2Speed = max;
  else if (right2Speed < -max)
    right2Speed = -max;
}

int op() {
  double delay = 5, extendedArmMaxSpeed = 30, extendedArmThresh = 400,
         accel = 0.6, turnMax = 70, fwdMax = 100, sideMax = 75;
  while (true) {
    // Axis values
